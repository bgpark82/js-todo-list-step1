...
class TodoList {
    ...
    onClick = (e) => {
        ...
        if (clickedClass != CLASS.TOGGLE && clickedClass != CLASS.DESTROY)
            return;

        const todoId = Number(e.target.closest("li").id);
    ...
    onKeypress = (e) => {
        ...
        switch (e.key) {
            case KEY_EVENT.ENTER:
                this.onEditTodo(Number($todo.id), title);
                this.isEditing = false;
                return;
    ...
    createTodoListDOM = (todos) => {
        return todos.reduce((html, todo) => html + todoListDOM(todo), "");
    };
